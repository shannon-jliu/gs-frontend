...
const AssignmentOperations = {
  getAllAssignments: dispatch => (
    () => {
      const success = data => AssignmentOperations.processAssignments(dispatch)(data, null)
  ...
  nextAssignmentSuccess: dispatch => (
    currAssignment => (
      ...
      assignments => {
        AssignmentOperations.processAssignments(dispatch)(assignments, currAssignment)
  ...
  processAssignments: dispatch => (
    (assignments, currAssignment) => {
      if (assignments.length > 0) {
        ...
        const ind = currAssignment ? currAssignment.get('currentIndex') + 1 : assignments.length
